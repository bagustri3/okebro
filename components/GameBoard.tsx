...
export default function GameBoard({ matchPlayers, onUpdatePlayer, onAddHistory, onUndo }: GameBoardProps) {
        ...
        {matchPlayers.map((mp) => (
          ...
          >
            <div className="flex flex-col items-center gap-2 mb-4">
              <h3 className="text-xl font-bold text-white">{mp.player_name}</h3>
              <div className="flex items-center justify-center px-4 py-12 w-full bg-gray-800 rounded-xl shadow-sm border border-gray-600">
                <span className="text-4xl font-bold bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">
                  {mp.current_count || 'Oke Bro'}
                </span>
              </div>
            </div>
            ...
            {mp.status === 'RW' && (
              <div className="mt-4 px-4 py-2 bg-gradient-to-r from-amber-500/20 to-orange-500/20 border-2 border-amber-500 rounded-xl flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-amber-300">RW Status Active</span>
              </div>
            )}
          </div>
        ))}
